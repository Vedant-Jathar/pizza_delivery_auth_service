...
describe('GET /auth/self', () => {
    ...
    describe('happy path', () => {
        it('should return user id if the access token is valid and not expired', async () => {
            ...
            const user = await userRepo.save({ ...userData, role: constants_1.Role.CUSTOMER });
    ...
    describe('sad paths', () => {
        it('should return status code 404 if user not found', async () => {
            const accesToken = jwks.token({ sub: '5666', role: constants_1.Role.CUSTOMER });
            const response = await (0, supertest_1.default)(app_1.default)
                .get('/auth/self')
        ...
        it('should return status 401 if acces token is not sent with the request', async () => {
            const response = await (0, supertest_1.default)(app_1.default).get('/auth/self');
